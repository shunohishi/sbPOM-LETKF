...
! Modified by S.Ohishi 2019.05 for budget term
! Modified by S.Ohishi 2020.04 for nuding term
! Modified by S.Ohishi 2020.11 for surface ensemble
!-------------------------------------------------------------------------
program main
  ...
  allocate(ens2d(im,jm,nmem))

  !Read model information(time,...,dvm)
  !write(*,*) "Read info"
  call read_info(dir,region,iyr,imon,iday,dt,im,jm,km, &
  ...
  do it=1,dt
     ...
     call create_nc("mean",dir,region,syr,smon,sday,iyr,imon,iday,im,jm,km,nmem,iswitch_budget)
     call create_nc("sprd",dir,region,syr,smon,sday,iyr,imon,iday,im,jm,km,nmem,iswitch_budget)
     call create_nc("eens",dir,region,syr,smon,sday,iyr,imon,iday,im,jm,km,nmem,iswitch_budget)
     
     !Write model information(time,...,dvm)
     call write_info("mean",dir,region,iyr,imon,iday,im,jm,km, &
          & time(it),z,zz,east_u,east_v,east_e,north_u,north_v,north_e, &
          & h,fsm,dum,dvm)
     call write_info("sprd",dir,region,iyr,imon,iday,im,jm,km, &
          & time(it),z,zz,east_u,east_v,east_e,north_u,north_v,north_e, &
          & h,fsm,dum,dvm)
     call write_info("eens",dir,region,iyr,imon,iday,im,jm,km, &
          & time(it),z,zz,east_u,east_v,east_e,north_u,north_v,north_e, &
          & h,fsm,dum,dvm)
  ...
  do ivar=1,50
     ...
     write(*,*) "Calculate Mean/Sprd of "//trim(var)//" in mesp_ens.out @ "//year//month//day
     ...
     do it=1,dt
        ...
        do imem=1,nmem
           ...
           elseif(23 <= ivar .and. ivar <= 50)then
              ! write(*,*) "Read ensemble"
        ...
        if(1 <= ivar .and. ivar <= 22)then
           ! write(*,*) "Write Mean"
           call write_ens("mean",var,dir,region,iyr,imon,iday,im,jm,1,mean2d)
           ! write(*,*) "Write Spread"
           call write_ens("sprd",var,dir,region,iyr,imon,iday,im,jm,1,sprd2d)
        elseif(23 <= ivar .and. ivar <= 50)then
           ! write(*,*) "Write Mean"
           call write_ens("mean",var,dir,region,iyr,imon,iday,im,jm,km,mean3d)
           ! write(*,*) "Write Spread"
           call write_ens("sprd",var,dir,region,iyr,imon,iday,im,jm,km,sprd3d)
        end if
        
        if(1 <= ivar .and. ivar <= 26)then
           ! write(*,*) "Write ensemble member"
           call write_ens("eens",var,dir,region,iyr,imon,iday,im,jm,nmem,ens2d)
        end if
...
subroutine read_var(ivar,var)
  ...
  if(ivar == 38) var="tiau"
  if(ivar == 39) var="dsdt"
  if(ivar == 40) var="sxadv"
  if(ivar == 41) var="syadv"
  if(ivar == 42) var="szadv"
  if(ivar == 43) var="sadv"
  if(ivar == 44) var="sxdif"
  if(ivar == 45) var="sydif"
  if(ivar == 46) var="szdif"
  if(ivar == 47) var="snudge"
  if(ivar == 48) var="siau"
  if(ivar == 49) var="aam"
  if(ivar == 50) var="kh"
...
subroutine read_ngrid(dir,region,iyr,imon,iday,im,jm,km)
  ...
  filename=trim(dir)//"/001/"//trim(region)//"."//year//month//day//".nc"
...
subroutine read_info(dir,region,iyr,imon,iday,dt,im,jm,km, &
     & time,z,zz,east_u,east_v,east_e,north_u,north_v,north_e, &
     & h,fsm,dum,dvm)
...
subroutine read_ens(var,dir,region,imem,iyr,imon,iday,it,im,jm,km,dat)
  ...
  filename=trim(dir)//"/"//mem//"/"//trim(region)//"."//year//month//day//".nc"
  ...
  if(status == nf_noerr)then
     if(km == 1)then
        status=nf_get_vara_real(ncid,varid,(/1,1,it/),(/im,jm,1/),dat)
     else
        status=nf_get_vara_real(ncid,varid,(/1,1,1,it/),(/im,jm,km,1/),dat)
     end if
  else
     dat(:,:,:)=0.
  end if
...
subroutine create_nc(mesp,dir,region,syr,smon,sday,iyr,imon,iday,im,jm,km,nmem,iswitch_budget)
  ...
  integer idims
  integer vdims(4)
  integer x_dimid,y_dimid,z_dimid,nmem_dimid,t_dimid
  integer length
  ...
  !Write filename
  filename=trim(dir)//"/"//trim(mesp)//"/"//trim(region)//"."//year//month//".nc"
  ...
  if(mesp == "eens")then
     !MEM
     status=nf_def_dim(ncid,'mem',nmem,nmem_dimid)
  end if
  ...
  call def_var_netcdf(ncid,'east_u',idims,vdims,varid, &
       & 'easting of u-points','metre', &
       & 'east_u north_u',.true.)
  call def_var_netcdf(ncid,'east_v',idims,vdims,varid, &
       & 'easting of v-points','metre', &
       & 'east_v north_v',.true.)
  call def_var_netcdf(ncid,'east_e',idims,vdims,varid, &
       & 'easting of elevation points','metre', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'north_u',idims,vdims,varid, &
       & 'northing of u-points','metre', &
       & 'east_u north_u',.true.)
  call def_var_netcdf(ncid,'north_v',idims,vdims,varid, &
       & 'northing of v-points','metre', &
       & 'east_v north_v',.true.)
  call def_var_netcdf(ncid,'north_e',idims,vdims,varid, &
       & 'northing of elevation points','metre', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'h',idims,vdims,varid, &
       & 'undisturbed water depth','metre', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'fsm',idims,vdims,varid, &
       & 'free surface mask','dimensionless', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'dum',idims,vdims,varid, &
       & 'u-velocity mask','dimensionless', &
       & 'east_u north_u',.true.)
  call def_var_netcdf(ncid,'dvm',idims,vdims,varid, &
  ...
  call def_var_netcdf(ncid,'el',idims,vdims,varid, &
       & 'surface elevation','metre', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'lhf',idims,vdims,varid, &
       & 'latent heat flux','W/m^2', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'shf',idims,vdims,varid, &
       & 'sensible heat flux','W/m^2', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'lwr',idims,vdims,varid, &
       & 'longwave radiation','W/m^2', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'swr',idims,vdims,varid, &
       & 'shortwave radiation','W/m^2', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'windu',idims,vdims,varid, &
       & 'zonal wind','m/s', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'windv',idims,vdims,varid, &
       & 'meridional wind','m/s', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'winds',idims,vdims,varid, &
       & 'wind speed','m/s', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'tauu',idims,vdims,varid, &
       & 'zonal wind stress','N/m^2', &
       & 'east_u north_u',.true.)
  call def_var_netcdf(ncid,'tauv',idims,vdims,varid, &
       & 'meridional wind stress','N/m^2', &
       & 'east_v north_v',.true.)
  call def_var_netcdf(ncid,'taus',idims,vdims,varid, &
       & 'magnitude of wind stress','N/m^2', &
       & 'east_v north_v',.true.)
  call def_var_netcdf(ncid,'qa',idims,vdims,varid, &
       & 'air specific humidity','g/kg', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'qs',idims,vdims,varid, &
       & 'surface saturated specific humidity','g/kg', &
       & 'east_e north_e',.true.)
  call def_var_netcdf(ncid,'ta',idims,vdims,varid, &
       & 'air temperature','degree C', &
       & 'east_e north_e',.true.)
  
  if(iswitch_ff == 1)then
     call def_var_netcdf(ncid,'evap',idims,vdims,varid, &
                   'evaporation','mm/day', &
                   'east_e north_e',.true.)
     call def_var_netcdf(ncid,'prep',idims,vdims,varid, &
                   'precipitation','mm/day', &
                   'east_e north_e',.true.)
     call def_var_netcdf(ncid,'river',idims,vdims,varid, &
                   'river discharge','mm/day', &
                   'east_e north_e',.true.)
     call def_var_netcdf(ncid,'eflux',idims,vdims,varid, &
                   'evaporation flux','m/s', &
                   'east_e north_e',.true.)
     call def_var_netcdf(ncid,'pflux',idims,vdims,varid, &
                   'precipitation flux','m/s', &
                   'east_e north_e',.true.)
     call def_var_netcdf(ncid,'rflux',idims,vdims,varid, &
                   'river discharge flux','m/s', &
                   'east_e north_e',.true.)
  end if
    
  if(iswitch_budget == 1)then
     call def_var_netcdf(ncid,'tsfc',idims,vdims,varid, &
          & 'surface heatflux forcing','degree C/day', &
          & 'east_e north_e',.true.)

     call def_var_netcdf(ncid,'ssfc',idims,vdims,varid, &
          & 'surface freshwater forcing','psu/day', &
          & 'east_e north_e',.true.)
  end if
  ...
  if(mesp == "eens")then
     ...
     call def_var_netcdf(ncid,'u',idims,vdims,varid, &
          & 'x-velocity','metre/sec', &
          & 'east_u north_u zz',.true.)
     call def_var_netcdf(ncid,'v',idims,vdims,varid, &
          & 'y-velocity','metre/sec', &
          & 'east_v north_v zz',.true.)
     call def_var_netcdf(ncid,'t',idims,vdims,varid, &
          & 'potential temperature','degree C', &
          & 'east_e north_e zz',.true.)
     call def_var_netcdf(ncid,'s',idims,vdims,varid, &
          & 'salinity','psu', &
          & 'east_e north_e zz',.true.)
  ...
  if(iswitch_budget == 1)then
     ...
     call def_var_netcdf(ncid,'tiau',4,vdims,varid, &
          & 'temperature for IAU method','deg C', &
          & 'east_e north_e zz',.true.)           
     call def_var_netcdf(ncid,'siau',4,vdims,varid, &
          & 'salinity for IAU method','psu', &
          & 'east_e north_e zz',.true.)
...
subroutine write_info(mesp,dir,region,iyr,imon,iday,im,jm,km, &
     & time,z,zz,east_u,east_v,east_e,north_u,north_v,north_e, &
     & h,fsm,dum,dvm)
  ...
  status=nf_inq_varid(ncid,"time",varid)
  status=nf_put_vara_real(ncid,varid,(/iday/),(/1/),time)
...
subroutine write_ens(mesp,var,dir,region,iyr,imon,iday,im,jm,km,dat)
