...
csh make_vcoord.csh ${WORKDIR} ${MEMBER} ${ENODE} ${EPROC}
set iexit=$?
if(${iexit} == 99)then
...
@ PROC = ${EPROC} * ${MEMBER}
@ THREAD = ${NPROC} / ${EPROC}
echo "Each Node:"${ENODE}
echo "Each Processor:"${EPROC}
...
csh ${CDIR}/submit_job.csh ${EPROC} ${MEMBER} ${THREAD} ${REGION} ${PROG} ${WORKDIR} \${PJM_JOBID}
...
while(${IMEM} <= ${MEMBER})
    ...
    if(-s sbPOM_${REGION}_${yyyy1}${mm1}${dd1}_iau${JOBID}.stats)then
	(mv -f sbPOM_${REGION}_${yyyy1}${mm1}${dd1}_iau${JOBID}.stats ${INFO}/ &)
    else
	(rm -f sbPOM_${REGION}_${yyyy1}${mm1}${dd1}_iau${JOBID}.stats &)
    endif

    (rm -f sbPOM_${REGION}_${yyyy1}${mm1}${dd1}_iau${JOBID}.out &)
    (rm -f sbPOM_${REGION}_${yyyy1}${mm1}${dd1}_iau${JOBID}.err &)
