   ...
   SUBROUTINE aerobulk_compute( calgo, zt, zu, sst, t_zt, &
      ...
      do j=1,jpj
         do i=1,jpi
            ...
            if(Tau_x(i,j) .ne. Tau_x(i,j))then
               write(*,*) "Tau_x"
               write(*,*) i,j
               write(*,*) Tau_x(i,j),Cd(i,j),XRHO(i,j),U_zu(i,j),XUblk(i,j)
            end if

            if(Tau_y(i,j) .ne. Tau_y(i,j))then
               write(*,*) "Tau_y"
               write(*,*) i,j
               write(*,*) Tau_y(i,j),Cd(i,j),XRHO(i,j),V_zu(i,j),XUblk(i,j)
            end if
