...
! [Celcius] -> {Kelvin]
      ...
      if(lmsm) then
        call rh2sh(airt,sstm,airh,airh_sh,im,jm)         !msm
      else
        do j=1,jm
          do i=1,im
            airh_sh(i,j)=airh(i,j)
          end do
        end do
      end if
      call sensibleh(qh,sstm,airt,windu,windv,im,jm,my_task)
      call latenth(qe,sstm,airt,airh_sh,windu,windv,im,jm,my_task)
      call longrad(lwrad,sstm,airt,airh_sh,cloud,im,jm,my_task)
...
! unit and direction changes
          wusurf(i,j)=-(wusurf(i,j))*1.e-4
          wvsurf(i,j)=-(wvsurf(i,j))*1.e-4
        end do
      end do
!
      return
