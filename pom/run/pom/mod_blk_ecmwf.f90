...
CONTAINS
   ...
   SUBROUTINE TURB_ECMWF( zt, zu, T_s, t_zt, q_s, q_zt, U_zu,       &
      ...
      do j=1,jpj
         do i=1,jpi
            if(ztmp1(i,j) == 1.)then
               func_h(i,j) = ztmp1(i,j) * (ztmp0(i,j) + 3.*ztmp2(i,j))
            else if(ztmp1(i,j) == 0.)then
               func_h(i,j) = (1.-ztmp1(i,j)) * ztmp0(i,j)*zu/(zu-ztmp2(i,j)*zi0*0.004*Beta0**3)
            else
               func_h(i,j) = (1.-ztmp1(i,j)) * ztmp0(i,j)*zu/(zu-ztmp2(i,j)*zi0*0.004*Beta0**3) &
                    &  +     ztmp1(i,j)   * (ztmp0(i,j) + 3.*ztmp2(i,j))
            end if
         end do
      end do
